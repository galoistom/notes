...
>[!note] 维塔利定理
>如果有界集$E$按照维塔利的意义被一闭区间$M$所覆盖, 则从$M$中可以选出有限个或可数个闭区间$\{d_k\}$, 使$d_k \cap d_i = \emptyset$, $m^*[E-\sum_kd_k]=0$.

其中依照维塔利的意义被$M$覆盖是指对于一个点集$E$, 由闭区间组成的集合$M$使得任一$x \in E$存在闭区间$d \subset M$满足: $x \in d$, $md<\epsilon$.
