...
>2. $A$ is integrally closed. (i.e. $\forall\alpha \in K=frac(A)$, if $\alpha$ is integral in $A$, then $\alpha \in A$)
...
Meanwhile, $\mathfrak{p}$ a prime ideal of $A$, then the localization $A_{\mathfrak{p}}$ is a discrete valuation ring. 
