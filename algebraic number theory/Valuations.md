...
1) $char(K)=0$. For $n$ sufficiently large, we hve the isomorphism $\log:U^{(n)}\rightarrow \mathfrak{p}^{n}=\pi ^{n}{\small\mathcal{O}}\simeq \small\mathcal{O}$. Since $\log$ and $\exp$ and $f(z)=(1+x)^{z}$ are continuous, this is a topological isomorphism of $\mathbb{Z}_{p}$-modules. Moreover, $\small\mathcal{O}=\mathbb{Z}_{p}\alpha_{1}\oplus\cdots\oplus \mathbb{Z}_{p}\alpha_{d} \simeq \mathbb{Z}_{p}^{d}$. Therefore $U^{(n)}\simeq \mathbb{Z}_{p}^{d}$. Since the index $(U^{(1)}: U^{(n)})$ is finite and $U^{(n)}$ is a finitely generated $\mathbb{Z}_{p}$-module of rank $d$, so is $U^{(1)}$. The torsion subgroup of $U^{(1)}$ is the group $\mu_{p^{a}}$ of roots of unity in $K$ of $p-power$ order. Using the main theorem on moudles over PID, there exists in $U^{(1)}$ a free, finitely generated, hence closed. $\mathbb{Z}_{p}$-submodule $V$ of rank $d$ such that $U^{(1)}=\mu_{p^{a}}$