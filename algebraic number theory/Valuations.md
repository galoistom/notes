...
1. $\small\small\mathcal{O}=\{x \in K:v(x)\geq 0\} = \{x \in K : |x|\leq 1\}$,
2. $\small\small\mathcal{O}^{*} = \{x \in K : v(x)=0\} = \{x \in K : |x|=1\}$,
3. $\mathfrak{p}= \{x \in K : v(x)>0\} = \{x \in K : |x|<1\}$.
4. $U=1+\mathfrak{p}=\left\{  x \in K : |1-x| < \frac{1}{q^{n-1}} \right\}$.
With $v(x) = -\log|x|$. 

So we are now able to change our view, let $\small\small\mathcal{O}$ is an integral domain with field of fraction $K$ and has property that for all $x \in K$, either $x \in \small\mathcal{O}$ or $x^{-1} \in \small\mathcal{O}$. Such a ring is called a *valuation ring* as it can be seen as a valuation if a field. We are also able to define the maximal ideal $\mathfrak{p}$ in the sence of $\{ x \in \small\mathcal{O} : x^{-1} \not\in \small\mathcal{O} \}$. In particular, an exponential valuation $v$ is called *descrete* if and only if $v(K^{*}) \simeq s\mathbb{Z}$ for some $s$, in other words, it admits a smallest prositive value $s$. One can easily normalize it (which is lettin $s=1$) then we are able to find a $\pi \in \small\mathcal{O}$ such that $v(\pi)=1$, and $(\pi)$ is then the ideal $\mathfrak{p}$ and $\pi$ is the *prime element*, and we are able to write all $x \in K$  in the form of $u\pi ^{m}$ where $u \in \small\mathcal{O}^{*}$. Moreover, we define $U^{(n)}=1+\mathfrak{p}^{n}=\left\{  x \in K^{*} : |1-x|< \frac{1}{q^{n-1}}  \right\}$

A simple fact is that $\mathfrak{p}^{n} / \mathfrak{p}^{n+1} \simeq \small\small\mathcal{O} / \mathfrak{p}$. 
...
Now with the help of the theorem, we can focous on the cases of nonarchimedean valuations. We first have a simple proposition $\widehat{\small\small\mathcal{O}} / \widehat{\mathfrak{p}} \simeq \small\mathcal{O} / \mathfrak{p}$. Moreover, we can see the completion as adding elements that is of the form of infinite power series $x=\pi ^{m}(a_{0}+a_{1}\pi+a_{2}\pi ^{2}+\cdots)$. As in the case of p-adic numbers, we are now able to acquire similiar result in general valuation theory. The construction is similiar, we first have $$
\small\small\mathcal{O} / \mathfrak{p} \stackrel{\lambda_{1}}{\longleftarrow} \small\mathcal{O} / \mathfrak{p}^{2} \stackrel{\lambda_{2}}{\longleftarrow} \cdots 
$$
and this gives  us a homomorphism $\small\small\mathcal{O} \rightarrow \lim\limits_{\longleftarrow} \small\mathcal{O} / \mathfrak{p}^{n}$, in fact, the mapping is a isomorphism and a homeomorphism. The same is true for $\small\mathcal{O}^{*} \rightarrow \lim\limits_{\longleftarrow} \small\mathcal{O}^{*} / U^{(n)}$. The proof is clear, and notice that $\small\mathcal{O}^{*}\simeq(\lim\limits_{\longleftarrow} \small\mathcal{O} / \mathfrak{p}^{n})^{*}$, then the rest is simple. 
...
Another interesting theorem conserns about the changes of valuations in field extension. Let $K$ be complete with respect to the valuation $|\ |$. Then $|\ |$ may be extended in a unique way to a valuation of any given algebraic extension $L | K$. This extension is given by the formula $|\alpha| = \sqrt[n]{ |N_{L|K}(\alpha)| }$. 

*Proof*:If $|\ |$ is archimedean, using the Ostrowski's theorem, we know immediately that the statement above is correct. So we will focous on the nonarchimedean part.
1. Existence: Let $\small\small\mathcal{O}$ be the valuation ring of $K$ and $\small\mathcal{O}$ is its integral closure in $L$. Then one has $\small\mathcal{O}=\{ \alpha \in L | N_{L|K}(\alpha) \in \small\mathcal{O} \}$. We already know that $\alpha \implies N_{L|K}(\alpha) \in \small\mathcal{O}$. Conversly, if $\alpha\in L^{*}$ and $N_{L|K}(\alpha) \in \small\small\mathcal{O}$. Let $f(x)=x^{d}+\sum ^{d-1}_{i=0}a_{i}x^{i}$ be the minimal polynomial of $\alpha$, then as $N_{L|K}(\alpha)=\pm a_{0}^{m} \in \small\mathcal{O}$, so $|a_{0}|\leq1$, hence $a_{0}\in \small\mathcal{O}$, so $f(x) \in \small\mathcal{O}[x]$, i.e. $\alpha \in \mathcal{O}$. One immediately check that the definition fits for all condition needed for a valuation, so it do exists.
2. Uniqueness: Suppose the contrary, let $|\ |'$ be another extension of $|\ |$. Let $\mathfrak{P}$ ($\mathfrak{P}'$) be the maximal ideal of $\mathcal{O}$ ($\mathcal{O}'$). We are going to proof that $\mathcal{O}=\mathcal{O}'$, in fact, we only need to proof that $\mathcal{O} \subseteq \mathcal{O}'$. Take $\alpha \in \mathcal{O} \backslash \mathcal{O}'$, with minimal polynomial $f$, then we know that all coefficents of $f$ are in $\small\mathcal{O}$ and $\alpha ^{-1} \in \mathfrak{P}'$, hence $1 \in \mathfrak{P}'$, a contradiction. Thus $\mathcal{O}=\mathcal{O}'$, i.e. $|\alpha|\leq1 \Longleftrightarrow |\alpha|'\leq1$, hence the two valuations are equivalent. 

# Local Field
Among all complete (nonarchimedean) valued fields, those arising as completions of a *global field* i.e. of a finite extension of either $\mathbb{Q}$ or $\mathbb{F}_{p}(t)$ have the most eminent relevance for number theorey. The valuation on such a completion is discrete and has a finite residue class field. In contrast to the global fields, all fields with are complete with respect to a discrete valuation and have a finite residue class field are called *local fields*. For such a local field, the normalized exponential valuation is denoted by $v_{\mathfrak{p}}$, and $|\ |_{\mathfrak{p}}$  denotes the absolute value normalized by $|x|_{\mathfrak{p}}=q^{-v_{\mathfrak{p}}(x)}$. 

**Proposition**: all local fields are precisely the finite extension of the fields $\mathbb{Q}_{p}$ and $\mathbb{F}_{p}((t))$. 

*Proof*:
A finite extension $K$ of $k=\mathbb{Q}_{p}$ or $k=\mathbb{F}_{p}((t))$ is again complete, with respect to the extended valuation $|\alpha|=\sqrt[n]{ |N_{K|k}(\alpha)| }$, which is obviously descrete. Since $K|k$ is of finite degree, so is ${\small\mathcal{K}} | \mathbb{F}_{p}$, as linear combation can be inherated from $K$, so we have $K$ is local. 
...
1) $char(K)=0$. For $n$ sufficiently large, we hve the isomorphism $\log:U^{(n)}\rightarrow \mathfrak{p}^{n}=\pi ^{n}{\small\mathcal{O}}\simeq \small\mathcal{O}$. Since $\log$ and $\exp$ and $f(z)=(1+x)^{z}$ are continuous, this is a topological isomorphism of $\mathbb{Z}_{p}$-modules. Moreover, $\small\mathcal{O}=\mathbb{Z}_{p}\alpha_{1}\oplus\cdots\oplus \mathbb{Z}_{p}\alpha_{d} \simeq \mathbb{Z}_{p}^{d}$. Therefore $U^{(n)}\simeq \mathbb{Z}_{p}^{d}$. Since the index $(U^{(1)}: U^{(n)})$ is finite and $U^{(n)}$ is a finitely generated $\mathbb{Z}_{p}$-module of rank $d$, so is $U^{(1)}$. The torsion subgroup of $U^{(1)}$ is the group $\mu_{p^{a}}$ of roots of unity in $K$ of $p-power$ order. Using the main theorem on moudles over PID, there exists in $U^{(1)}$ a free, finitely generated, hence closed. $\mathbb{Z}_{p}$-submodule $V$ of rank $d$ such that $U^{(1)}=\mu_{p^{a}}\times V \simeq \mathbb{Z} / p^{a}\mathbb{Z}\oplus \mathbb{Z}_{p}^{d}$, both algebraically and topologically. 
2) $char(K)=p$. We have $K\simeq \mathbb{F}_{q}((t))$ and $U^{(1)}=1+\mathfrak{p}=1+t\mathbb{F}_{q}[[t]]$. Consider the continuous homomorphism $g_{n}:\mathbb{Z}_{p}^{f}\rightarrow U^{(n)}$, $g_{n}(a_{1},\cdots,a_{f})=\prod_{i=1}^{f}(1+\omega_{i}t^{n})^{a_{i}}$ where $\omega_{1}, \cdots, \omega_{f}$ be a basis of $\mathbb{F}_{q}|\mathbb{F}_{p}$. Then one notice that $U^{(m)}=g_{n}(p^{s}\mathbb{Z}_{p}^{f})U^{(m+1)}$ and, for $\alpha=(a_{1}, \cdots, a_{f}) \in \mathbb{Z}_{p}^{f}$, $\alpha \not\in p\mathbb{Z}_{p}^{f} \Longleftrightarrow g_{n}(p^{s}\alpha)\not\in U^{(m+1)}$. We then construct $g=\prod_{gcd(n,p)=1}g_{n}:A=\prod_{gcd(n,p)=1}\mathbb{Z}_{p}^{f}\longrightarrow U^{(1)}$, one may notice that as $U^{(m)}=g_{n}(p^{s}\mathbb{Z}_{p}^{f})U^{(m+1)}$, and $g_{n}(\mathbb{Z}_{p}^{g})\subseteq g(A)$, we have $g(A)$ is dense in $U^{(1)}$. Since $A$ is compact and $g$ continuous, $g$ is infact surjective. On the other hand, we proof that $g$ is injective, i.e. $g(\xi)\neq 1$ for all $\xi \in A$. Let $\xi=(\cdots,\alpha_{n}, \cdots)\in A\backslash\{ 0 \}$, and suppose $\alpha_{n}=p^{s}\beta_{n}$, where $\beta_{n} \in \mathbb{Z}_{p}^{f}\backslash p\mathbb{Z}_{p}^{f}$, and $s=s(\alpha) \in \mathbb{Z}_{\geq 0}$, set $m=m(\alpha_{n})=np^{s}$. Then $g_{n}(\alpha_{n}) \in U^{(m)} \backslash U^{(m+1)}$. Let $n$ to be the one making $m(\alpha_{n})$ smallest (as $gcd(n,p)=1$, so $m(\alpha_{n})$ have to be distinct). We have $g(\xi)\equiv g_{n}(\alpha_{n})\not\equiv 1 \pmod{U^{(m+1)}}$. 

A Corollary:
If the natural number $n$ is not divisible by the chararteristic of $K$, the one finds the following indices for the subgroups of $n-th$ powers $K^{*n}$ and $U^{n}$ in the multiplicative group $K^{*}$ and in the unit group $U$: 
$$
(K^{*}:K^{*n}) = n(U:U^{n}) = \frac{n}{|n|_{\mathfrak{p}}} \sharp \mu_{n}(K)
$$

**Definition**: A **henselian field** is a field with a nonarchimedean valuation $v$ whose valuation ring $\mathcal{O}$ satisfies Hensel's lemma. One also calls the valuation $v$ or the valuation ring $\mathcal{O}$ henselian. 
