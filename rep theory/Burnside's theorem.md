...
2. First take an irreducible representation $(V,\rho)$ with character $\chi$. Then consider the map $\rho :\mathbb{Z}[G]\rightarrow GL(V)$, and take the center $\overline{\rho}:cent.\mathbb{Z}[G]\rightarrow \mathbb{C}$. One notice that $cent.\mathbb{Z}[G]$ is generated by $\sum_{g\in C}g$, so the image is finitly generated as $\mathbb{Z}$-module, hance an algebric integer. So $\frac{|C|\chi(c)}{\chi(1)}$ is an algebric integer.
