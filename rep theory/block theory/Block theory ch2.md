...
3) Let $\Phi: Hom_{\mathcal{C}}(\mathcal{G}(-),-) \simeq Hom_{\mathcal{D}}(-,\mathcal{F}(-))$ be an adjunction isomorphism with unit $f$ and counit $g$. Then $\Phi(V,U)(\phi) = \mathcal{F}(\phi) \circ f(V)$ for any object $U$ in $\mathcal{C}$, any object $V$ in $\mathcal{D}$ 