...
**Proprsition**:
Let $G$ be a finite group, let $A$ be a $G$-algebra over $k$, and let $H$ be a subgroup of $G$. For any $a \in A^{H}$ and $b \in A^{G}$ we have $\mathrm{Tr}_{H}^{G}(a)b= \mathrm{Tr}_{H}^{G}(ab)$  