...
Now we proceed with proofing that $l$ belongs to $V_{p}$, i.e. $l$ is of the form $\sum_{H}a_{H}Ind_{H}^{G}(f_{H})$, with $a_{H}\in A$ and $f_{H}\in R(H)$. How do we proof it? One recall the [[Artin's theorem]]: $g=\sum_{C}Ind_{C}^{G}(\theta_{C})$, then for a general character $\chi$ with integer value, $f=g\chi=\sum_{C} Ind_{C}^{G}(\theta_{C})\chi=\sum_{C}Ind_{C}^{G}(\theta_{C}\cdot Res_{C}^{G}\chi)\in$