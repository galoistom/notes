...
*proof*: $\frac{1}{a}\sum_{g}\overline{T(g)}(\phi(a)\cdot r_{A}(g)-\theta_{A})=\frac{1}{a}\sum_{g}(\phi(a)r_{A}(g)-\theta_{A}(g))$, note that $\frac{1}{a}\sum_{g}\theta_{A}=\phi(a)$, and $\frac{1}{a}\sum_{g}r_{A}(g)=a$, so it is clear that is is orthogonal to the unit representation. Now use the proposition above, we have $\sum_{A\subset G}Ind_{A}^{G}(\theta_{A})=g$, so we only have to proof that $\sum_{A\subset G}Ind_{A}^{G}(\phi(a)r_{A})=gr_{G}$. In fact, $Ind_{A}^{G}r_{A}(x)=\frac{1}{a}\sum_{y\in G,\,yxy^{-1}\in A}r_{A}(yxy^{-1})$, one immediately notice that $Ind_{A}^{G}r_{A}(x)$ is nonzero if and only if $x=1_{G}$, as $yxy^{-1}=1_{A}$ <==> $x=1_{G}$. In this case $Ind_{A}^{G}r_{A}(1_{G})=g$, now  with $a|g$, we have $\sum_{A\subset G}Ind_{A}^{G}\phi(a)r_{A}(1_{G})=g\sum_{a|n}\phi(a)=g^{2}$, so $\sum_{A\subset G}Ind_{A}^{G}\phi(a)r_{A}=gr_{G}$. Then we are done. 

**Remark**: This proposition is useful in the following case (Aramata-Brauer):
Let $F$ be a finite extension of the number field $E$, and let $\Phi = \zeta_{F}(s) / \zeta_{E}(s)$, be the quotient of their zeta functions. It is known that $\Phi$ is meromophic in the entire complex plane. Now suppose that $L / K$ is a Galois extension with Galois group $G$. then the formula aboves the identity 
$$
