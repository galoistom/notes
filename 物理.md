...
定义分子的自由程为两次碰撞之间的路程, 事实上, 我们更加关注平均自由程 $\overline{\lambda}= \frac{\overline{v}}{\overline{Z}}$, 其中 $\overline{v}$ 为分子平均速度, $\overline{Z}$ 为分子的平均碰撞频率. 事实上, $\overline{Z}= \frac{N_{col}}{\Delta t} = n\sigma \overline{u}$, 其中 $\sigma=\pi d^{2}$ 为分子的碰撞截面. 