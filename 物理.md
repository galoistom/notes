...
定义粒子流密度 $j_{n}$ 为单位时间内面积上扩散的粒子数, 即 $j_{n}=\Delta n / \Delta t$. 事实上, 粒子流密度 $j_{n}$ 与粒子束密度梯度 $d_{n} / d_{z}$ 成正比, 即 $j_{n} = -D \frac{dn}{dz}$, 其中 $D$ 称为自扩散系数. In particular, 如果在与扩散的方向垂直的截面 