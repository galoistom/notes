...
# 热学
理想气体状态方程: $\frac{pV}{T}=vR=\frac{N}{N_{A}}R=Nk_{B}$. 修正: $\left( p+\frac{v^{2}a}{V^{2}} \right)(V-vb)=vRT$. 
其中 $R\simeq 8.31J\cdot K^{-1}\cdot mol^{-1}$; $k_{B}=R / N_{A}\simeq1.38\times10^{-23}J\cdot K^{-1}$. 

体(等温)膨胀系数: $\alpha=\frac{1}{V}\left( \frac{\partial V}{\partial T} \right)_{p}$, 
等压强系数: $\beta=\frac{1}{p}\left( \frac{\partial p}{\partial T} \right)_{V}$,
等压缩系数: $\kappa=-\frac{1}{V}\left( \frac{\partial V}{\partial p} \right)_{T}$.
$p\beta\kappa=\alpha$. 

压强的统计解释: 考虑和边缘碰撞的分子, 动量改变 $2mv_{x}$, 然后基于速率分布, 考虑在 $\Delta t$ 时间内的动量改变, 然后全部变为压力, 得到 $p=\frac{1}{3}\cdot \frac{N}{V}\cdot \overline{\vec{p}\cdot \vec{v}}$. 同时注意到在宏观下有$p=\frac{N}{V}k_{B}T$, 因此我们有 $\overline{\frac{1}{2}mv^{2}}=\frac{3}{2}k_{B}T$, 以及能量在各个方向应当相等.

道尔顿分压定理: 设一个容器体积为 $V$, 总压强为 $p$, 温度为 $T$, 总粒子数为 $N$, 质量为 $M$. 容器中盛有 $n$ 中不同种类的气体, 对于其中的任意一种气体, 体积为 $V$, 压强为 $p_{i}$, 温度为 $T$, 粒子数为 $N_{i}$, 质量为 $M_{i}$. 
$$
p_{i}=\frac{N_{i}}{V}k_{B}T
$$

高斯积分表
![[Pasted image 20251006124706.png]]

麦克斯韦速度分布律: $f_{M}(\mathbf{v})=\left( \frac{m}{2\pi k_{B}T} \right)^{3/2}\cdot e^{-\frac{m\mathbf{v}^{2}}{2k_{B}T}}$.
麦克斯韦速率分布律: $F(v)=\left( \frac{m}{2\pi k_{B}T} \right)^{3/2}\cdot e^{-mv^{2}}\cdot4\pi v^{2}$. 

泻流速度: $\Gamma=\frac{1}{4}n\overline{v}$, 泻流速度分布律: $\frac{4\pi}{\overline{v}}\cdot v^{3}\cdot\left( \frac{m}{2\pi k_{B}T} \right)^{3/2}\cdot e^{-\frac{m\mathbf{v}^{2}}{2k_{B}T}}$. 

玻尔兹曼密度分布: $n_{B}(\vec{r})=n_{0}e^{-U(\vec{r})/k_{B}T}$. 
In particular: 重力场下 $n(z)=n_{0}e^{-\frac{mgz}{k_{B}T}}$, 离心力场下 $n(r)=n_{0}e^{\frac{m\omega ^{2}r^{2}}{2k_{B}T}}$

麦克斯韦-玻尔兹曼能量分布律: $f_{MB}(\vec{v},\vec{r})=n_{0}\left( \frac{m}{2\pi k_{B}T} \right)^{3/2}\cdot e^{-\epsilon/k_{B}T}$. 

能均分定理: 热平衡状态下, 物质分子的每个自由度具有相同的平均动能 $\frac{1}{2}k_{B}T$. 内能: 分子动能和势能的总和 $\overline{\epsilon}=\frac{1}{2}(t+r+2s)k_{B}T$. 

玻尔兹曼熵: $S=k_{B}\ln \Omega$. 

热传导中的热流量$\Phi$ 正比例于温度梯度 $dT / dz$ 和传热面积 $\Delta S$. 即 $\Phi= \frac{\Delta Q}{\Delta t}=- \kappa \frac{dT}{dz}\Delta S$, 其中比例系数 $\kappa$ 称为热导率或热导系数. 定义单位时间内在单位面积上流过的热量为热流密度, 记为 $\phi=-\kappa \frac{dT}{dz}$. 

定义粒子流密度 $j_{n}$ 为单位时间内面积上扩散的粒子数, 即 $j_{n}=\Delta n / \Delta t$. 事实上, 粒子流密度 $j_{n}$ 与粒子束密度梯度 $d_{n} / d_{z}$ 成正比, 即 $j_{n} = -D \frac{dn}{dz}$, 其中 $D$ 称为自扩散系数. In particular, 如果在与扩散的方向垂直的截面 $j_{n}$ 处处相同, 则菲克扩散定理又可以由质量流量表示为 $J = \frac{\Delta M}{\Delta t}= -D \frac{d\rho}{dz}S$, 其中 $S$ 为发生扩散的截面面积, $\rho$ 为密度. 

定义分子的自由程为两次碰撞之间的路程, 事实上, 我们更加关注平均自由程 $\overline{\lambda}= \frac{\overline{v}}{\overline{Z}}$, 其中 $\overline{v}$ 为分子平均速度, $\overline{Z}$ 为分子的平均碰撞频率. 事实上, $\overline{Z}= \frac{N_{col}}{\Delta t} = n\sigma \overline{u}$, 其中 $\sigma=\pi d^{2}$ 为分子的碰撞截面. 特别的, 我们在两分子相对运动的情况下 $\overline{u}=\sqrt{ 2 }\overline{v}$, 从而 $\overline{\lambda}=\frac{1}{\sqrt{ 2 }n\sigma}$. 

实验表明, 定常层流中, 任一截面积为 $\Delta S$ 的截面所受的粘性力正比例于流速梯度和留层面积, 即 $\vec{f}=-\eta \frac{d\vec{u}}{dz}\Delta S$, 其中比例系数 $\eta$ 称为流体的黏度或黏性系数, 同时 $u=u(z)$ 表示定常层流的速度分布. 

在微观上 $\eta = \frac{1}{3}\rho \overline{v}\overline{\lambda}$, $\kappa=\frac{1}{3}\rho \overline{v}\overline{\lambda}c_{v}$, $D=\frac{1}{3}\overline{v}\overline{\lambda}$. 

热力学中简单的研究对象: 孤立体系, 处于平衡态. 
复杂的研究对象: (热力学过程) 从一个平衡态变成另一个平衡态经历的过程. 

准静态过程: 进行足够缓慢, 以至于系统连续经过的每个中间态都可以近似的堪称平衡态的过程.

- 等容过程: $dU=dQ-dW=C_{V}dT$.
- 等压过程: $dU=dQ-dW=C_{p}dT-pdV=C_{p}dT+d(pV)=C_{V}dT$. 定义态函数焓: $H=U+pV$. 


$C_{p}=C_{v}+vR$. 注意到 $U=\frac{1}{2}(t+r+2s)vRT= \frac{i}{2}pV$, $H=U+vR$, 或者 $dU=C_{V}dT+\left((C_{p}-C_{V})\cdot \left(\frac{\partial T}{\partial V}\right)-p \right)dV$. 
