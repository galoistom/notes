...
Write $m=m_{0}\cdot m_{\infty}$, where $m_{0}=\prod_{p\ finite}p^{n_{p}}$ and $m_{\infty}=\prod_{p\ infinite}p^{n_{p}}$. When all $n_{p}$ are $0$, write $m=1$. Let $I_{K}(m)$ be the group of all fractional $\mathcal{O}_{K}$-ideals relatively prime to $m$ and let $P_{K,1}(m)\subseteq I_{K}(m)$ be generated by $\alpha \mathcal{O}_{K}$. 