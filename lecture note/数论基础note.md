...
*Proof*: $L /\mathbb{Q}$ Galois, then choose $\alpha \in \mathcal{O}_{L}\cap \mathbb{R}$ generating $L^{\tau}$ over $\mathbb{Q}$. Then $L=K(\alpha)$, $f_{n}=f_{\alpha}=min_{\mathbb{Q}}(\alpha)$. Then deg $f_{n}$ is $h(-4n)$. 