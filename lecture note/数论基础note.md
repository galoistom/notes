...
Let $K$ be a number field. "modules" in $K$ formal product $m=\prod_{p}p^{n_{p}}$, over primes of $K$, where 
1. $n_{p}\geq0$ and $n_{p}\neq0$ for a most finitely many primes $p$. 
2. $n_{p}=0$ whenever $p$ is a complex infinite prime.
3. $n_{p}\leq1$ whenever $p$ is a real infinite prime.

Write $m=m_{0}\cdot m_{\infty}$, where $m_{0}=\prod_{p\ finite}p^{n_{p}}$ and $m_{\infty}=\prod_{p\ infinite}p^{n_{p}}$. When all $n_{p}$ are $0$, write $m=1$. Let $I_{K}(m)$ be the group of all fractional $\mathcal{O}_{K}$-ideals relatively prime to $m$ and let $P_{K,1}(m)\subseteq I_{K}(m)$ be generated by $\alpha \mathcal{O}_{K}$, $\alpha \in \mathcal{O}_{K}$ and $\alpha\equiv1\pmod{m_{0}}$ and $\sigma(\alpha)>0$ for real infinite prime $\sigma$ dividing $m$. In fact, $[I_{K}:P_{K,1}(m)]<+\infty$. 

A subgroup $H\subseteq I_{K}(m)$ is called a **conjaguence subgroup for $m$**, if $P_{K,1}(m)\subseteq H\subseteq I_{K}(m)$ and "$I_{K} / H$" is the **genrealized ideal class group**. 